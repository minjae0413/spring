 ...
 */
public class Step1 {
	public void step1() { //클래스명과 같은 이름의 메서드를 생성자 메서드 자동으로 만들어집니다.
		//중요한 이유는 다른 클래스에서 Step1을 객체로 만들 때 생성자 메서드가 필요.
		//Step1 aaa = new Step1();
		//자바앱에서는 객체를 생성자 메서드로 만들때 필수
		//단, 스프링에서는 @Inject으로 객체를 만들어서 사용.
	}
	...
	public static void main(String[] args) {
		...
		//메서드명이 같습니다. 단,로드된 파라미터 타입, 갯수 다른 메서드를 호출=오버로드 (loading...)
	...
	private static void printMember(String[] names, int[] ages, String[] phoneNums) {
		...
		for(int i=0;i<dataLength;i++) {//고전적 for반복문
			System.out.println(i+"입력하신 회원의 이름은 "+names[i]+"님, 나이는  "+ages[i]+", 폰번호는 "+phoneNums[i]);
		}
	}
