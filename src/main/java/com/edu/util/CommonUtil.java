...
public class CommonUtil {
	...
	public void profile_upload(String user_id, HttpServletRequest request, MultipartFile file) throws IOException {
		// TODO 프로필 이미지는 보안이 필요한 폴더가 아닌, resources폴더에 업로드 처리. 서버의 경로 필요
		...
		if(!makeFolder.exists()) {
			makeFolder.mkdir();//여기서 신규 폴더가 생성 됩니다.
		}
		//1개에 파일이 1000개 이상이면, 조회 속도가 엄청 느려짐.
		//년월폴더를 생성 후, 해당년월에 업로드된 파일은 년월 폴더로 관리.
		byte[] fileData = file.getBytes();
		File target = new File(makeFolder,user_id+".png");//user_id는 PK이기 때문에 기존 파일이 있다면, 덮어쓰면서 저장됩니다.
