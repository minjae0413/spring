...
public class AdminController {
	...
	public String board_insert(@RequestParam("file")MultipartFile[] files,BoardVO boardVO) throws Exception {
		...
		//AttachVO테이블에 가로 데이터를 세로 데이터로 입력하기 위해서...
	...
	public String updateMember(HttpServletRequest request, MultipartFile file, MemberVO memberVO, PageVO pageVO) throws Exception {
		//프로필 이미지 처리 추가
		if(!file.getOriginalFilename().isEmpty()) {//신규 파일이 있으면 실행, 없으면 건너뜀.
		...
		if(!rawPassword.isEmpty()) {//수정폼에서 암호 입력값이 비어있지 않을때만 아래 로직실행.
			...
			String encPassword = passwordEncoder.encode(rawPassword);
			memberVO.setUser_pw(encPassword);
			//스프링시큐리티 내장 클래스에서 user_pw(admin1234)와 password(해시값)비교함수
	...
	//URL요청 경로는 @RequestMapping 반드시 절대경로로 표시. 개발자 A 작업
	...
	//메인페이지 또는 대시보드에 최신 테이블 리스트를 출력하는 방법 2가지(위,model사용)
	//아래, <c:import방식 : 최신 게시물 용도로 사용 //페이지안에서 컴파일된 다른 페이지를 불러올 수 있음. 개발자 B 작업
