...
public class MemberServiceImpl implements IF_MemberService {
	...
	//헤로쿠 클라우드에 30분 휴면 상태를 깨우는 기능 추가->root-context에서 지정한 스케줄러에서 20분 간격으로 호출됨
	public void herokuJobMethod() throws Exception {
		//한국시간 월~금 오전 8~23시까지 (미국시간 0-14) 헤로쿠앱에 20분간격으로 접근
		//헤로쿠 컨테이너가 러닝할 수 있는 무료 시간 1달 700시간 정도 됩니다. 시간이 넘으면 먹통(1달내)
		//주말에 이력서보시는 분이 없기때문에
		String urlStr = "https://jominjae-spring5.herokuapp.com/";
		URL url = new URL(urlStr);
		HttpURLConnection urlConnection = (HttpURLConnection) url.openConnection();//접속 객체만들기
		urlConnection.setUseCaches(false);//접속시 캐시사용없이 무조건 새로 고침하겠다고 명시, 컨테이너가 계속 떠있습니다.(세션도 유지됨)
