--19장 사용자추가시(CreateWorkSpace) 오라클데스크탐X, 대신
-- 웹프로그램을 사용 (http://127.0.0.1:9000/apex/f?p=4950)
-- ,SQL플러스X,
--15장 PK생성시 자동으로 2개 생성 NOT NULL(빈값방지), UNIQUE(NO중복)
-- 제약조건(constraint)이 자동생성, INDEX(테이블)도 자동생성(검색시 중요)
...
DESC dept;--단, 작은자리 큰자리로 이동 문제없음.
ALTER TABLE dept MODIFY(depno NUMBER(4));
--11 서브쿼리
--단일행서브쿼리 필드값을 비교할때, 비교하는 값인 단일한(필드값)
--다중행서브쿼리 테이블값을 select쿼리로 생성(레코드형식)
--10장.테이블 조인 2개테이블 연결해서 결과를 구하는 예약어
--댓글개수 구할때,
...
WHERE e.ename = 'SCOTT';
--조인과 그룹을 이용해서 댓글 카운터도 출력하는 게시판 리스트 만들기
...
--DB셋팅에서 대소문작구분해서 사용할지, 구분하지 않을지 셋팅
SELECT R.* FROM(
...
--라운드함수(반올림) 소수점기준. round(10.05,2)소수점 2째 반올림
...
--위 AVG함수를 where조건에 사용못할때 서브 쿼리를 이용합니다.
SELECT MAX(sal)
, MIN(sal) 
...
--위 쿼리는 테이블을 복제하는 명령
--위처럼 쿼리를 실행 dept테이블과 구조와 내용이 똑같은 테이블이 생성
...
--DCL명령어인 커밋이 필수입니다.
COMMIT;--데이터베이스쿼리직접입력한 결과는 반드시 커밋을 해줘야지만 실제 저장이 됩니다. 커밋을 하지 않으면, 여기만 보이고, 다른 곳은X
SELECT * FROM dept02 ORDER BY deptno;
--DELETE는 레코드 1줄을 지우는 명령
DELETE FROM dept02;--이렇게 사용하면 모든 레코드가 삭제됨 주의요.
DELETE FROM dept02 WHERE deptno >= 0;--모두 삭제 where반드시 포함
--DROP table 테이블명은 테이블 자체를 물리적으로 없애는 명령
DROP TABLE dept02;--드롭 테이블 커밋 없이 바로 적용됨.
...
ROLLBACK;--DCL문 롤백은 마지막 커밋 바로전까지 되돌아갑니다.
